    ...
    if not st.session_state.tarama_yapildi or not st.session_state.sonuclar:
        ...
        if rot and rot["giren"]:
            ...
            with rc1:
                ...
                for s in rot["giren"]:
                    st.markdown(ui.temiz_html(f"""<div style="color:#94A3B8;font-size:0.72rem;padding:2px 0">{s['sektor']} <span style="color:{'#10B981' if s['momentum']>=0 else '#EF4444'}">%{s['momentum']:+.0f}</span> · {s['lider'] or '-'}</div>"""), unsafe_allow_html=True)
            with rc2:
                if rot["cikan"]:
