...
import bildirim as bd    # Telegram bildirimi (token yoksa sessiz no-op)
...
def calistir():
    ...
    with open(MD, "w", encoding="utf-8") as f:
        f.write("\n".join(L))
    # ---- Telegram bildirimi (yalnızca YENİ kayıtta; token yoksa sessiz) ----
    if yeni:
        try:
            yv = pz.yillik_vol(xu_df["Close"].values)
            msg = bd.mesaj_kur(tarih_str, bugun_durus, m["reel"], m["politika"],
                               m["enflasyon"], pos["w"], yv)
            gonderildi = bd.gonder(msg)
            print("Telegram:", "gönderildi" if gonderildi else "atlandı (token yok/hata)")
        except Exception as e:
            print("Telegram hata:", type(e).__name__)
